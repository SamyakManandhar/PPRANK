...
/**
 * Servlet implementation class RecommendRegister
 */
@WebServlet("/RecommendRegister")
public class RecommendRegister extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = recommendDao.registerRecommend(r);
		System.out.println("res=" + result);
		if (result > 0) {
			RequestDispatcher rd = request.getRequestDispatcher("user.jsp");
			request.setAttribute("message", "User registered successfully");
			request.setAttribute("username", r.getUsername());
			request.setAttribute("fromusername", r.getFromusername());
