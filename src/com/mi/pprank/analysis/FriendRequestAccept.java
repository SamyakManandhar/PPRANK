...
/**
 * Servlet implementation class FriendRequestAccept
 */
@WebServlet("/FriendRequestAccept")
public class FriendRequestAccept extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			rs = stmt.executeQuery("SELECT username, auth FROM friend");
			while (rs.next()) {
				String category = rs.getString(1);
				int marks = rs.getInt(2);
				dataset.addValue(marks, "FriendRequest", category);
			}
